...
//获取详细的项目信息
    ...
    public String index(Integer id, Model model, HttpSession session){
        AppResponse<ProjectDetailVo> vo = projectServiceFeign.findProjectInfo(id);
...
//项目回报查询
    ...
    public String findReturnInfo(@PathVariable("projectId") Integer projectId, @PathVariable("returnId") Integer returnId, Model model, HttpSession session){
        ...
        //根据项目回报编号，获取项目回报信息
        AppResponse<ReturnPayConfirmVo> appResponse = projectServiceFeign.findReturnInfo(returnId);
        //获取项目回报信息
        ...
        //根据项目发起方id，获取项目发起方名称
        AppResponse<UserRespVo> memberServiceAppResponse = userServiceFeign.findUser(projectDetailVo.getMemberid());
    ...
    public String confirmOrder(@PathVariable("num") Integer num,Model model, HttpSession session){
        ...
        AppResponse<List<UserAddressVo>> addressResponse = userServiceFeign.findAddressList(accessToken);
