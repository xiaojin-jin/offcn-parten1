...
import com.offcn.user.vo.resp.UserRegistVo;
...
public class UserLoginController {
    ...
    public AppResponse<Object> sendCode(String phoneNo) {
        ...
        redisTemplate.opsForValue().set(phoneNo, code, 10, TimeUnit.DAYS);
        //3、短信发送构造参数
        Map<String, String> querys = new HashMap<String, String>();
        querys.put("mobile", phoneNo);
        querys.put("param", "code:" + code);
        querys.put("tpl_id", "TP1710262");//短信模板
        //4、发送短信
    ...
    public AppResponse<Object> regist(UserRegistVo registVo) {
        ...
        if (!StringUtils.isEmpty(code)) {
            //redis中有验证码
            boolean b = code.equalsIgnoreCase(registVo.getCode());//equalsIgnoreCase()忽略大小写
            if (b) {
                //2、将vo转业务能用的数据对象
